...
def main():
    ...
    st.title("🏁 Prédiction résultat Grand Prix 🏁")
    ...
    if not futur_events.empty:
        ...
        if next_event['Session4DateUtc'] < actual_date < next_event['Session5DateUtc']:
            ...
            with col1:
                st.metric(label="🎯 MAE Rank", value=f"{mae_rank:.2f}", delta="- (Places)", delta_color="inverse")
                st.caption("Erreur moyenne en positions")

            with col2:
                st.metric(label="🧮 MAE Raw", value=f"{mae_raw:.2f}", delta="- (Maths)", delta_color="inverse")
            ...
            with col3:
                ...
                st.caption(f"Basé sur le Round {last_round}")
